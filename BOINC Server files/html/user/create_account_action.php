...
if (!is_valid_email_addr($new_email_addr)) {
    show_error(tra("Invalid email address: you must enter a valid address of the form name@domain.tld"));
}
...
if (!$min_passwd_length) $min_passwd_length = 24;
...
if (strlen($passwd)<$min_passwd_length) {
    show_error(
        tra("New password is too short: minimum password length is %1 characters. Use a password manager to generate a secure password!", $min_passwd_length)
    );
}
